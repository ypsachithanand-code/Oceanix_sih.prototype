  ...
  return (
    <div className="glass-panel p-5 rounded-3xl border border-[#162c3f] space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-2xl border border-[#162c3f] bg-[#071722] p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm text-[#cfe5ff]">
              <Camera className="h-4 w-4 text-[#50d6f9]" />
          ...
          <label className="mt-4 flex h-56 cursor-pointer items-center justify-center rounded-2xl border border-dashed border-[#2d4d67] bg-gradient-to-br from-[#0b2235] to-[#08131d] p-4 text-center transition hover:border-[#50d6f9]">
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
            ...
            {imagePreview ? (
              ...
              <div className="space-y-3">
                <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#0f3550] text-[#50d6f9]">
        ...
        <div className="rounded-2xl border border-[#162c3f] bg-[#071722] p-4">
          <div className="flex items-center gap-2 text-sm text-[#cfe5ff]">
            <Sparkles className="h-4 w-4 text-[#50d6f9]" />
            <span>Detection preview</span>
          </div>

          {isProcessing ? (
            <div className="mt-4 flex h-44 items-center justify-center rounded-2xl border border-[#162c3f] bg-[#081c2d] text-sm text-[#9BB7C9]">
              Analyzing the specimen image...
            </div>
          ) : detection ? (
            <div className="mt-4 space-y-3">
              <div className="rounded-2xl border border-[#50d6f9]/25 bg-[#0d2c42] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-lg font-semibold text-white">{detection.name}</p>
                    <p className="text-xs text-[#78d1ff]">{detection.family}</p>
                  </div>
                  <div className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold text-emerald-300">
                    {detection.confidence}
                  </div>
                </div>

                <div className="mt-3 space-y-2 text-sm text-[#cfe5ff]">
                  <p>
                    <span className="text-[#9BB7C9]">Habitat:</span> {detection.habitat}
                  </p>
                  <p>
                    <span className="text-[#9BB7C9]">Signal:</span> {detection.highlight}
                  </p>
                  <p>
                    <span className="text-[#9BB7C9]">Source:</span> {detection.fileName}
                  </p>
                  <p>
                    <span className="text-[#9BB7C9]">File size:</span> {detection.fileSize}
                  </p>
                </div>
              </div>
            ...
            <div className="mt-4 rounded-2xl border border-[#162c3f] bg-[#08131d] p-4 text-sm text-[#9BB7C9]">
